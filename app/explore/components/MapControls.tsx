...
export default function MapControls({
  ...
  return (
    <div className="relative border-t border-[#1a1a1a] bg-[#050505]/80 backdrop-blur-sm safe-area-bottom">
      <div className="accent-line absolute top-0 left-0 right-0" />

      <div className="flex items-center px-3 py-2.5 md:px-5 md:py-3 md:justify-center overflow-x-auto scrollbar-hide">
        <div className="flex items-center gap-2 md:gap-3 shrink-0">
          <div className="flex items-center gap-1.5 mr-1 md:mr-2 shrink-0">
            <Layers className="w-4 h-4 text-[#4a4540]" />
            <span className="text-xs text-[#4a4540] uppercase tracking-[0.1em] font-medium hidden sm:inline">
              Data Layers
            </span>
            {activeOverlays.size > 0 && (
              <span className="px-1.5 py-0.5 rounded-full bg-[#14b8a6]/10 text-[10px] text-[#14b8a6] font-medium">
                {activeOverlays.size}
              </span>
            )}
          </div>
          {OVERLAY_CONFIGS.map((config) => {
            ...
            return (
              <button
                ...
                className={`relative overflow-hidden flex items-center gap-1.5 md:gap-2 px-2.5 md:px-3 py-1.5 rounded text-xs md:text-sm font-medium border transition-colors whitespace-nowrap shrink-0 ${
                  active
          ...
          {selectedState && (
            <>
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 py-1 rounded bg-[#14b8a6]/10 border border-[#14b8a6]/20 text-xs md:text-sm text-[#14b8a6] hover:bg-[#14b8a6]/20 transition-colors shrink-0"
              >
